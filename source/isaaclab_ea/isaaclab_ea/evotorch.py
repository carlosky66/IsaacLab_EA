...
class Runner:
    def __init__(
        self, env_problem, algorithm_name: str, algorithm_cfg: dict, logger_cfg: dict
    ):
...
class EvoTorchNEVecEnvWrapper(NEProblem):
    def __init__(self, env: ManagerBasedRLEnv, problem_cfg):
        ...
        self.obs_dim = (
            obs_dim_raw[0] if isinstance(obs_dim_raw, tuple) else int(obs_dim_raw)
        )

        # Get action dimension (handle tuple or int)
        act_dim_raw = self.unwrapped.action_manager.total_action_dim
        self.act_dim = (
            act_dim_raw[0] if isinstance(act_dim_raw, tuple) else int(act_dim_raw)
        )
    ...
    def _batched_forward_pass(self, obs, population_weights):
        ...
        for name, shape, numel in zip(
            self.param_names, self.param_shapes, self.param_numels
        ):
